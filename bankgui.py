    ...
    for name in all_accounts:
        if name == login_name:
            ...
            if login_password == password:
                ...
                #Labels
                Label(account_dashboard, text="Account Dashboard", font=('Algerian',18)).grid(row=0,sticky=N,pady=10)
...
#Image import
img = Image.open('bank2.png')
img = img.resize((270,270))
img = ImageTk.PhotoImage(img)

#Labels
Label(master, text = "               Bank Management System               ",font=('Algerian',24)).grid(row=0,column = 0,sticky=N,pady=10)
Label(master, image=img).grid(row=2,sticky=N,pady=15)

#Buttons
Button(master, text="Register", font=('Calibri',12),width=20,bg='green',command=register).grid(row=3,sticky=N)
Button(master, text="Login", font=('Calibri',12),width=20,bg='green',command=login).grid(row=4,sticky=N,pady=10)
name = Label(text = "~Vishal Rote").place(x = 600, y = 435) 
